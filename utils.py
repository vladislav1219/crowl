...
def get_dbname(basename, lastId): # TIM --> Générer nom bdd : project_name_idCrawl
    """
    Generates a database name by adding id at the end
    """
...
# TIM --> Database connector
def db_connected_crowl_interface() :
    import pymysql
...
# TIM --> Stats dumper
def dump_stats(crawlId, stats, insert=True, update=False) :
    db = db_connected_crowl_interface()
    cur = db.cursor()
    if insert :
        cur.execute("""INSERT INTO crawl_stats (`crawl_id`, `enqueued`, `dequeued`, `response_bytes`, `response_received_count`) 
        VALUES ({}, {}, {}, {}, {} )""".format(crawlId, stats['scheduler/enqueued'], stats['scheduler/dequeued'], stats['downloader/response_bytes'], stats['response_received_count']))
    if update :
        cur.execute("""UPDATE `crawl_stats` SET `enqueued`={},`dequeued`={},`response_bytes`={}, `response_received_count`={} WHERE crawl_id={}""".format(stats['scheduler/enqueued'], stats['scheduler/dequeued'], stats['downloader/response_bytes'], stats['response_received_count'], crawlId))
    db.commit()
    db.close()
    return(True)

def quickSql(sql, fetch=False) :
    fetched=""
    db = db_connected_crowl_interface()
    cur = db.cursor()
    cur.execute(sql)
...
def getDomain(url) :
    ...
    try :
        sub, domain, tld = extract(url)  # prints abc, hostname, com
        return(domain+"."+tld)
    except :
        return(False)
