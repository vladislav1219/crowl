...
class Crowler(CrawlSpider):
    name = 'Crowl'
    http_user = 'ceciestuntest'
    http_pass = 'ceciestuntest'
    ...
    def __init__(self, url, links=False, content=False, depth=5, exclusion_pattern=None, *args, **kwargs):
        ...
        # TIM --> Get last ID for further update
        db = db_connected_crowl_interface()
        cur = db.cursor()
        cur.execute("SELECT lastId FROM lastId")
        global lastId
        ...
        if exclusion_pattern:
            # Process exclusion_pattern to allow a list of excluded regexes
                exclusion_pattern_list = []
                [exclusion_pattern_list.append(i) for i in exclusion_pattern.splitlines()]
                # Set link extraction rules
                self._rules = [
                Rule(LinkExtractor(allow='.*'+domain+'/.*', deny=exclusion_pattern_list), callback=self.parse_url, follow=True)
            ]
        else:
            self._rules = [
        ...
        # TIM --> Setting counters for stat dumping
        global firstTimeStats
        firstTimeStats = True
        global reqCount
        reqCount = 0
    ...
    def parse_item(self, response):
        ...
        # TIM --> Insert or update stats
        global firstTimeStats
        global reqCount

        if firstTimeStats :
            dump_stats(lastId, self.crawler.stats.get_stats(), update=False, insert=True)
            firstTimeStats = False
        else :
            if reqCount > 5 :
                dump_stats(lastId, self.crawler.stats.get_stats(), update=True, insert=False)
                reqCount = 0
    ...
    def closed(self, reason):
        ...
        # TIM --> Update crawl_stats
        stats = self.crawler.stats.get_stats()
        dump_stats(lastId, stats, update=True, insert=False)
        db = quickSql("""UPDATE crawls SET `state`='{}' WHERE id={}""".format(stats['finish_reason'], lastId))
